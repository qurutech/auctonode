...
export default new Vuex.Store({
  state: {
    ...
    currentLoggedInUser:  JSON.parse(localStorage.getItem("wavesKeeperData")) || {},
  ...
  getters: {
    auctoNodeOwnersCount(state) {
      return state.auctoNodeOwners.length ? state.auctoNodeOwners.length - 1 : 0
    },
    verifiedAuctoNodeOwners(state) {
      return state.auctoNodeOwners.length ? state.auctoNodeOwners.filter(function verifiedOwner(owner) {
        return state.verifiedAccounts.includes(owner.address) && owner.address !== state.auctAddress;
      }) : []
    }
  },
  mutations: {
  ...
  actions: {
    ...
    getAuctoNodeOwners(context) {
      fetch(`https://nodes.wavesnodes.com/assets/${context.state.auctAssetId}/distribution`).then(response => response.json())
      .then((response) =>{
        ...
        for (let [key, value] of Object.entries(auct)) {
            let arrayEntry = {
              ...
              status: context.state.verifiedAccounts.includes(key) ? "Verified" : "Unverified"
            }
        ...
        const auctoNodeOwners = auctTokenHolders.filter((auct) => {
        ...
        const totalStakedAuct =  auctoNodeOwners.filter(auct => auct.address !== '3P7H2Zqt4NK3J5Q2wF8gjcLw9187gC1bbAG').reduce((total, auct) => {
